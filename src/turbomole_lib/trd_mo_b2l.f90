...
!    if (line2(1:8) == "nbf(mo)") then
!      read(line2(10:),'(i10)') nmo
!    end if
!    !read irreps
...
if (method == "cc2") then
  ...
  ctypden = 'adcp2'
else if (method == "ccs" .or. method == "cis" .or. method == "cis(d)" .or. method == "cis(di)") then
  ctypden = 'ccs'
end if
...
do irrep=1, nirrep
  ...
  do istate=1, nstate
    ...
    if (method == "adc(2)") then
      allocate(trdenright_vector(((nmo*nmo)/2)+nmo/2))
      do i=1,(nmo*nmo)/2+nmo/2
        trdenright_vector(i) = 1.987654321
      end do
      allocate(trdenright(nmo,nmo,nspin))
      write(filename,'(a,"-tm0f-",i1,a,"-",i3.3,".mo")') trim(ctypden),imult,trim(ityp),istate
    ...
    else if (method == "cc2" .or. method == "ccs" .or. method == "cis" .or. method == "cis(d)" .or. method == "cis(di)") then
      allocate(trdenleft_vector(((nmo*nmo)/2)+nmo/2))
      do i=1,(nmo*nmo)/2+nmo/2
        trdenleft_vector(i) = 1.987654321
      end do
      allocate(trdenleft(nmo,nmo,nspin))
      write(filename,'(a,"-tmf0-",i1,a,"-",i3.3,".mo")') trim(ctypden),imult,trim(ityp),istate
      INQUIRE(FILE=trim(filename),EXIST=exists)
      if (exists) then
